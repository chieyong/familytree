-- Migratie 17 — Roepnaam (call_name).
--
-- Een persoon kan meerdere voornamen hebben (doopnamen, of een meerdelige
-- voornaam zoals "Mey Yen"). given_names houdt de VOLLEDIGE voornamen; de nieuwe
-- kolom call_name is de ROEPNAAM die als hoofd-label in de boom getoond wordt.
-- De frontend vult de roepnaam eerst af met de eerste voornaam, maar je kunt 'm
-- overschrijven (daarom een eigen kolom i.p.v. afgeleid givenNames[0]).
--
-- Alleen het leespad hoeft mee: _build_graph geeft 'callName' terug. Schrijven
-- gaat via een gewone UPDATE op persons (RLS ongewijzigd).
--
-- Idempotent: add column if not exists + create or replace function.

alter table persons add column if not exists call_name text;
