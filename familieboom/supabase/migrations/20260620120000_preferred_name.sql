-- Migratie 15 — Voorkeursnaam: welke naam als hoofd-label in de boom getoond
-- wordt ('native' = eigen schrift, 'nickname' = bijnaam). NULL = de volledige
-- (officiële) naam, het standaardgedrag. _build_graph geeft het veld mee.

alter table persons add column if not exists preferred_name text
  check (preferred_name in ('native', 'nickname'));
