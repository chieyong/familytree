-- Migratie 33 — Meerdere bijnamen per persoon.
--
-- Een persoon kan meerdere bijnamen hebben ("Joop", "de Lange", "Oompje"). De
-- oude scalar-kolom `nickname` wordt aangevuld met een array `nicknames` (eerste
-- = de primaire, die als boom-label kan dienen — dezelfde "eerste is primair"-
-- conventie als given_names).
--
-- Additief en laag-risico: de scalar `nickname` blijft bestaan en wordt door de
-- frontend gesynchroniseerd op nicknames[0], zodat functies die nog de scalar
-- lezen (add_relative, voorstel-toepassing, imports) correct blijven werken.
-- Alleen _build_graph geeft voortaan de array terug — met terugval op de scalar
-- voor rijen die (nog) alleen die gezet hebben (bv. net via add_relative).
--
-- Idempotent: add column if not exists / update alleen lege rijen / create or
-- replace function.

alter table persons add column if not exists nicknames text[] not null default '{}';

-- Backfill: bestaande scalar-bijnaam wordt de eerste (en enige) in de array.
update persons
  set nicknames = array[nickname]
  where nicknames = '{}' and nickname is not null and nickname <> '';

-- ─────────────────────────────────────────────────────────────────────────
-- _build_graph: identiek aan migratie 28, met één wijziging — 'nickname' (scalar)
-- wordt 'nicknames' (array). Terugval: staat de array leeg maar de oude scalar
-- gevuld, dan geven we [nickname] terug (net-aangemaakte personen via add_relative).
-- ─────────────────────────────────────────────────────────────────────────
  ...
  persons_json as (
    select coalesce(jsonb_agg(
      case when not i.hidden then jsonb_strip_nulls(jsonb_build_object(
        ...
        'nicknames', case
          when i.nicknames <> '{}' then to_jsonb(i.nicknames)
          when i.nickname is not null and i.nickname <> '' then jsonb_build_array(i.nickname)
        end,
